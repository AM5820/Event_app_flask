...
class Event(db.Model):
	...
	def __repr__(self):
		return f"name:{self.name}, content:{self.content},category:{self.category},img:{self.image_file}"
...
def upload_image():
	...
	if form.validate_on_submit():
	    ...
	    event = Event(name=form.name.data,address=form.address.data,content=form.content.data,
	    	category=form.category.data,image_file=file.filename,start_date=form.startDate.data,end_date=form.endDate.data)
...
def update_event(event_id):
	...
	if form.validate_on_submit():
		...
		#event.image_file = form.photo.data
